...
url: str = 'https://rpwnfrlgbvebtitmlfqa.supabase.co'
key: str = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InJwd25mcmxnYnZlYnRpdG1sZnFhIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDI5MDU5NjIsImV4cCI6MjA1ODQ4MTk2Mn0.uuuxAfJ4wr8f31YU3F_kozvn92CMlk4lJG13a2L7-rY'
supabase: Client = create_client(url, key)
SUMO_HOME = os.environ.get("SUMO_HOME", os.path.join(
    os.path.dirname(os.path.abspath(__file__)), ".."))
result_date_time = datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
try:
    basestring
...
class Builder(object):
    ...
    def __init__(self, data, local):
        ...
        self.tmp = None
        if local:
            now = data.get("outputDir",result_date_time)
...
if __name__ == "__main__":
    ...
    if args.testOutputDir is not None:
        ...
        server.serveforever()
    print("result_date_time: ", result_date_time)
    response = (
                supabase.table("simulations")
                .insert({"path": result_date_time})
                .execute()
    )
    print(response)
